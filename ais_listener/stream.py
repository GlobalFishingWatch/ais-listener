...
class MessageStream(object):
    ...
    def run(self):
        logger.info(f"Listening {self.protocol} socket on {self.hostname}:{self.port}")
    ...
    def handle(self):
        ...
        size = len(list(lines))
        if size > 0:
            logger.info(f"{self.protocol}: Received {size} messages.")

    def _read_messages(self):
        yield from self._read_from_queue()

    def _read_from_queue(self, max_messages=1000):
